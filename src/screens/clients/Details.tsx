    ...
    swal({
      ...
      if (confirm) {
        deleteRanch(ranch.id)
          .then(() => {
            swal('', 'Propriedade removida com sucesso!', 'success')
          })
          .then(fetch)
          .catch((e: Error) => swal('', e.message, 'error'))
      }
    ...
    >
      <>
        ...
        {!!client?.email && !!client?.document && (
          <p>
            <Button text="Enviar e-mail de senha" onClick={sendPasswordEmail} />
          </p>
        )}
            ...
            <tbody>
              {ranches.map((ranch) => (
                <tr key={ranch.id}>
                  ...
                  <td>
                    <BiTrash size={15} onClick={() => removeRanch(ranch)} />
                  </td>
                </tr>
              ))}
              {!ranches.length && (
                <tr>
                  <td colSpan={6}>Nenhuma propriedade cadastrada</td>
                </tr>
              )}
            </tbody>
          </>
